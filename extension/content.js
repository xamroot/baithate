...
function scrapePage(){
	if(loadedUrl !== window.location.toString()){
		...
		var targetUrlAction = document.createElement("script");
		targetUrlAction.innerHTML = `
			function getSummary(url) {
				$.ajax({
					type: "GET",
					url: "https://blooming-tor-41068.herokuapp.com/?target_url=" + url + "",
		...
		document.body.prepend(targetUrlAction);
		readyToLoad = true;
		killAll();
		return;
	}
		...
		$.ajax({
			...
			success: function(data){
				...
				for (let i=0; i<x.length; ++i) {
					...
					res = elementToAppend(titleElements[currentDomOffset + i])
					res[0].appendChild(containerElem);
					...
					tippy('#containerElemBox' + i, {
						content: val + `% chance of clickbait <br/> 
								<div style='text-align:center;' id='innerContentElem${i}'> 
									...
									<br />
									<a ${hidden ? "style='display:none;'" : ""} onclick='getSummary("${link}")'>Click to see article summary?</a>
								</div>`,
...
function GoogleNews(titleElement){
	return [titleElement.parentElement.parentElement.parentElement.parentElement, titleElement.parentElement.parentElement.parentElement.href];
}


function BuzzfeedHome(titleElement){
	return [titleElement.parentElement.parentElement.parentElement, titleElement.href];
}

function HackerNews(titleElement){
	return [titleElement.parentElement, titleElement.href];
}
